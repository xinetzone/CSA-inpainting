...
class CSAFunction(torch.autograd.Function):
    ...
    def forward(ctx, input, mask, shift_sz, stride, triple_w, flag, nonmask_point_idx, mask_point_idx, flatten_offsets, sp_x, sp_y):
        ...
        for idx in range(ctx.bz):
            ...
            _, conv_enc, conv_new_dec, _, known_patch, unknown_patch = Nonparm.buildAutoencoder(
                inpatch.squeeze(), False, False, nonmask_point_idx, mask_point_idx,  shift_sz, stride)
                ...
                for j in range(kbar_w):
                    ...
                    if(check.sum() >= 1):
                        ...
                        if ind_laten == 0:
                            ...
                            kbar[:, :, i, j] = torch.unsqueeze(
                                in_attention[ind_laten], 0)
                        elif ind_laten != 0:
                            little_value = unknown_region.clone()
                            ininconv = out_new[ind_laten - 1].clone()
                            ininconv = torch.unsqueeze(ininconv, 0)

                            value_2 = little_value * \
                                (1 / (little_value.norm(2) + 1e-8))
                            conv_enc_2 = nn.Conv2d(
                                256, 1, kernel_size=1, stride=1, bias=False)
                            ...
                            at_final_new = at_value_m / \
                                (at_value_m + vamx_mask[ind_laten])
                            at_final_ori = vamx_mask[ind_laten] / \
                                (at_value_m + vamx_mask[ind_laten])
                            out_new[ind_laten] = (
                                at_final_new) * out_new[ind_laten - 1] + (at_final_ori) * known_region
                            in_attention[ind_laten] = in_attention[ind_laten-1] * \
                                at_final_new.item()
                            in_attention[ind_laten, correct_ch] = in_attention[ind_laten,
                                                                               correct_ch]+at_final_ori.item()
                            kbar[:, :, i, j] = torch.unsqueeze(
                                in_attention[ind_laten], 0)
                        ind_laten += 1
                    else:
    ...
    def backward(ctx, grad_output):
        ...
        W_mat_all = Variable(ctx.Tensor(
            ctx.bz, spatial_size, spatial_size).zero_())
        for idx in range(ctx.bz):
            ...
            grad_swapped_weighted = torch.mm(
                W_mat_t, grad_swapped_all[idx].view(c, -1).t())

            # Then transpose it back
            grad_swapped_weighted = grad_swapped_weighted.t().contiguous().view(1,
                                                                                c, ctx.h, ctx.w)
